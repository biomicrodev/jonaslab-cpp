...
def make_buffer_array(array: numpy.ndarray, buffer: int) -> numpy.ndarray:
...
def blend_arrays(
    arrays: List[Tuple[numpy.ndarray, Tuple[float, ...]]], alpha1=0.5, alpha2=0.5
) -> numpy.ndarray:
...
class MakePaddedMask(cellprofiler.module.Module):
    ...
    def run(self, workspace: cellprofiler.workspace.Workspace) -> None:
        ...
        input_mask: cellprofiler.object.Objects() = workspace.object_set.get_object(
            input_mask_name
        )
