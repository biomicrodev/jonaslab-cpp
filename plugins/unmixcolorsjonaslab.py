...
# Choices in CellProfiler cannot contain colons for some reason
...
def estimate_absorbance() -> Union[numpy.ndarray, None]:
    ...
    def compute_absorbance(image_filepath: str) -> Union[numpy.ndarray, None]:
...
def get_absorbances(
    microscope: str, output: cellprofiler.setting.SettingsGroup
) -> numpy.ndarray:
...
def get_inverse_absorbances(
    microscope: str, outputs: List[cellprofiler.setting.SettingsGroup]
):
    absorbances = numpy.array([get_absorbances(microscope, o) for o in outputs])
    return numpy.linalg.pinv(absorbances)


def deconvolve(image: numpy.ndarray, inv_abs: numpy.ndarray) -> numpy.ndarray:
    # A small correction is needed to avoid log of zero
    eps = 1.0 / 256.0 / 2.0
    image += eps
...
custom values for the stain's red, green and blue absorbance.
    ...
    def add_image(self, can_remove: bool = True) -> None:
...
""",
    ...
    def prepare_settings(self, setting_values: List[Any]) -> None:
    ...
    def run(self, workspace: cellprofiler.workspace.Workspace) -> None:
        ...
        """
        # Prepare inputs
        input_image_name: str = self.input_image_name.value
        input_image: cellprofiler.image.Image = workspace.image_set.get_image(
            input_image_name, must_be_rgb=True
        )
        if self.show_window:
            workspace.display_data.input_image = input_image.pixel_data
            workspace.display_data.outputs = {}

        # Deconvolve for each dye
